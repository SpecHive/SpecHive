...
export function ErrorTimelineChart({ data, loading, metric }: ErrorTimelineChartProps) {
  const { chartData, seriesKeys, labelMap } = useMemo(() => {
    if (!data || data.series.length === 0) {
      return { chartData: [], seriesKeys: [], labelMap: new Map<string, string>() };
    }
    ...
    return {
      ...
      seriesKeys: data.series.map((s) => s.errorGroupId),
      labelMap: new Map(data.series.map((s) => [s.errorGroupId, s.title])),
    };
  }, [data, metric]);
